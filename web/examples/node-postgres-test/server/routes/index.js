...
router.get('/api/v1/todos', (req, res, next) => {
  ...
  pg.connect(connectionString, (err, client, done) => {
    ...
    const query = client.query('SELECT * FROM iridium ORDER BY id DESC LIMIT 30;');
...
router.get('/api/v1/coords', (req, res, next) => {
  const results = [];
  // Get a Postgres client from the connection pool
  pg.connect(connectionString, (err, client, done) => {
    // Handle connection errors
    ...
    // SQL Query > Select Data
    const query = client.query('SELECT latitude, longitude FROM iridium ORDER BY id DESC LIMIT 1;');
    // Stream results back one row at a time
    query.on('row', (row) => {
      results.push(row);
    });
...
router.post('/api/v1/todos', (req, res, next) => {
  ...
  pg.connect(connectionString, (err, client, done) => {
    ...
    var hex = data.data.toString();//force conversion
    var str = '';
    for (var i = 0; i < hex.length; i += 2)
        str += String.fromCharCode(parseInt(hex.substr(i, 2), 16));
 
    client.query('INSERT INTO iridium(imei, momsn, transmit_time, latitude, longitude, cep, data) values($1, $2, $3, $4, $5, $6, $7)',
