...
SpheroCollection.prototype.connect = function(port) {
  var self = this;
  ...
  if (port) {
    return setupConnection(port);
  }

  // try and connect by sp listing
  var portRegex = /cu.*sphero|^com/i;

  serialport.list(function (err, ports) {
    ports.filter(function(port){
      return portRegex.test(port.comName);
...
var Sphero = function(dev){
  ...
  ['open', 'error', 'close', 'end'].forEach(function (eventName) {
    this.dev.on(eventName, this.emit.bind(this, eventName));
  }.bind(this));

  this.incomingBuffer = null;

  this.dev.on('data', function (data) {
    if (Buffer.isBuffer(this.incomingBuffer)) {
      var oldBuffer = this.incomingBuffer;
      this.incomingBuffer = new Buffer(oldBuffer.length + data.length);
      oldBuffer.copy(this.incomingBuffer);
      data.copy(this.incomingBuffer, oldBuffer.length);
    }
    else {
      this.incomingBuffer = new Buffer(data.length);
      data.copy(this.incomingBuffer);
    }
    this._checkIncomingBuffer();
  }.bind(this));
...
Sphero.prototype.setRGBLED = function setRGB(r, g, b, persist, callback) {
  var data = new Buffer([r & 0xff, g & 0xff, b & 0xff, (persist && 0x01) | 0]);
  this._send(this.device.sphero.DID, this.device.sphero.commands.SET_RGB_LED.CID, data, callback);
  return callback;
};
